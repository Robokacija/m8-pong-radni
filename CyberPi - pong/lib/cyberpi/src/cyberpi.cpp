...
void CyberPi::_on_lcd_thread(void *p)
{
    while (true)
    {
        if (xSemaphoreTake(_render_ready, 3) == pdTRUE)
        {
...
Sprite CyberPi::create_text(char *chars, uint16_t color, uint8_t font_size)
{
...
void CyberPi::read_char(Sprite *bitmap, int x, int y, float w, float h, uint8_t *buffer, float font_width, float font_height, bool elongate, uint16_t color)
{
    if (buffer)
    {
...
void CyberPi::set_bitmap(uint8_t x, uint8_t y, Sprite *bitmap)
{
...
char *CyberPi::num_to_text(int num)
{
    int len = String(num).length();
    char *cStr = new char;
    cStr[len + 1]; // String.length() does not include the null terminator
